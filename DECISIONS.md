...
## Reverted: a clock deadline is not by itself coercion
Decision: `_COERCION` does not match a bare clock deadline such as "before 5 PM". A pattern for it was added, measured, and then removed. msg_022 ships as `notify`/`payment`.
Alternatives considered: Keeping it. It force-muted msg_022 — a society-group payment reminder carrying a link and a request to send the receipt back in the chat — and nothing else across the 110, with zero effect on the 30 labelled rows.
Why: A payment reminder that names an hour is ordinary collector language in a society or school group, and coercion in this gate is meant to be a *threatened consequence* ("account will be blocked", "service stops today"), not any stated cutoff. Pairing a clock with an existing lure reaches too far into legitimate traffic: the sender of msg_022 is a real `member` of a genuine society maintenance group, and muting a resident's payment reminder as `scam` mislabels a person rather than an attacker. Where such a message differs from a routine one is in urgency, which the `action` column already expresses. Keeping the pattern out also keeps `_COERCION` conceptually clean — every remaining entry names a consequence, not a time.
Trade-off / what we gave up: If the hidden labels do treat msg_022 as risk, we lose that row, and the same shape (deadline plus an in-chat payment instrument) would go uncaught elsewhere. The counter-evidence is on the record: u_044 is a member and not an admin, the real admins direct residents to the society app or office QR, and u_044's only two priors with this user were both dismissed and muted-after. See CHECKLIST.md §9 for the full case on both sides. The measured negative result from that work stands regardless — the obvious pattern `due today` force-muted 8 of 10 ordinary bill reminders and was rejected before the clock form was even considered.
